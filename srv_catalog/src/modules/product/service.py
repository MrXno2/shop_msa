...
class ProductService:
    ...
    async def create_product(self, req_data: ProductCreateSchema) -> None:
        ...
        try:
            ...
            raise HTTPException(400, "Invalid category_id or duplicate data") from None
    ...
    async def get_product_list(self, query_params: ProductListQuerySchema) -> ProductListResponseSchema:
        products, total = await self.product_repo.get_products_with_filters(query_params)

        product_schemas = [ProductResponseSchema.model_validate(product) for product in products]

        return ProductListResponseSchema(
            items=product_schemas, total=total, limit=query_params.limit, offset=query_params.offset
        )
