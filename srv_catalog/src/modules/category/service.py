...
class CategoryService:
    ...
    async def add_category(self, category_data: CategoryRequestSchema):
        new_category = CategoryORM(name=category_data.name, description=category_data.description)
        try:
            await self.category_repo.add_category(category_data=new_category)
            await self.db.commit()
        except IntegrityError:
            await self.db.rollback()
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "Invalid category_id or duplicate data") from None
