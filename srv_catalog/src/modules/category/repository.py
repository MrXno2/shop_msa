...
class CategoryRepository:
    ...
    async def del_category(self, uuid_category: UUID) -> None:
        await self.db.execute(delete(CategoryORM).where(CategoryORM.uuid == uuid_category))
    ...
    async def get_category(self, uuid_category: UUID) -> CategoryORM | None:
        result = await self.db.execute(select(CategoryORM).where(CategoryORM.uuid == uuid_category).limit(1))
