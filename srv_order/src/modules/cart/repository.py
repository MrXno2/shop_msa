...
class CartRepository:
    ...
    async def del_product(self, uuid_user: UUID, uuid_product: UUID) -> None:
        await self.db.execute(
            delete(CartORM).where(CartORM.uuid_user == uuid_user, CartORM.uuid_product == uuid_product)
        )

    async def del_all_product(self, uuid_user: UUID) -> None:
        await self.db.execute(delete(CartORM).where(CartORM.uuid_user == uuid_user))
    ...
    async def get_product(self, uuid_user: UUID, uuid_product: UUID) -> CartORM | None:
        result = await self.db.execute(
            select(CartORM).where(CartORM.uuid_user == uuid_user, CartORM.uuid_product == uuid_product).limit(1)
        )
