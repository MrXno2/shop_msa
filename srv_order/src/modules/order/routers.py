...
@router.post("/create")
async def create_order(order_service: OrderServiceDep, payload=Depends(is_validity_token)) -> None:
...
async def pay_for_order(order_service: OrderServiceDep, id_order: int, payload=Depends(is_validity_token)) -> None:
    uuid_user = payload.get("uuid")
