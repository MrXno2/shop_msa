    ...
    async def deduct_from_stock(
        ...
        async with db_session() as db:
            ...
            resp_rabbit = RabbitResponseCatalogSchema(
                id_order=data.id_order, uuid_user=data.uuid_user, status_order_type=OrderStatusEnum.CREATED
            )
            success = True
            for product in data.products:
                ...
                if update_product_stock.price != product.price or actual_sale != cached_sale:
                    success = False
                    break
            ...
            await rabbit_catalog_order.publish(
                "catalog_order.handle_stock_deduction_result", resp_rabbit.model_dump(mode="json")
            )
