...
from core_app.rabbit import rabbit_all_notification, rabbit_payment_order
...
class RabbitPaymentOrderService:
    async def payment_order(self, message: aio_pika.IncomingMessage):
        async with db_session() as db:
            try:
                wallet_repo = WalletRepository(db)
                ...
                logger.critical(f"INVALID MESSAGE SCHEMA: {e}, body: {message.body[:200]}")
                return
                ...
                logger.exception(f"Unexpected error: {e}, data: {message.body[:200]}")
                await db.rollback()
