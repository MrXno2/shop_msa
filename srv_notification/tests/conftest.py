...
from srv_notification.src.main import app

from core_app.settings import settings

TEST_DATABASE_URL = make_test_database_url(
    settings.POSTGRES_URL_SERV_NOTIFICATION, "shop_notification_test"
)
...
async def client(db_session: AsyncSession) -> AsyncClient:
    ...
    mock_rabbit.start = AsyncMock()
    mock_rabbit.stop = AsyncMock()
    mock_rabbit.consumer = MagicMock()

    with patch("srv_notification.src.main.rabbit_all_notification", mock_rabbit):
